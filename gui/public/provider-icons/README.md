...
### `devin-http`: fallback tile

`devin-http` keeps the initials tile even though `devin.svg` exists. The two are
separate registry entries — `devin` is the account provider that imports an
installed Devin CLI credential or falls back to Auth0 browser sign-in, while
`devin-http` is the key-mode entry pointing at the same undocumented Cascade
backend — and the dashboard resolves a mark by provider id, so a shared asset
would render two ids as one provider. Distinct tiles are the accurate state. If
the two ever collapse back into one entry, delete this section and let both ids
map to `devin.svg`.
