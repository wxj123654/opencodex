...
export const PROVIDER_REGISTRY_EXTENDED: readonly ProviderRegistryEntry[] = [
  ...
  // unpublished, so no cost entry is asserted.
  {
    ...
    // The GLM Chat surface rejects `developer` (HTTP 400 code 1214); record the fold so the
    // native passthrough and the alias destination both normalize instead of failing upstream.
    foldDeveloperRoleToSystem: true,
  ...
  // Originally proposed in #536 by @Lucinegogo.
  {
    ...
    adapter: "openai-chat",
    authKind: "key",
    // Same GLM Chat surface as `zai`: `developer` is rejected with HTTP 400 code 1214.
    foldDeveloperRoleToSystem: true,
