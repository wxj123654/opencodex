...
import { fetchDevinQuotaLive } from "../devin-usage";
...
/**
 * `devin-http` seat management — the provider already holds the session token for its
 * chat path, so the same credential is queried directly. A resolution failure or a
 * failed probe degrades to no row rather than an error.
 */
async function fetchDevinHttpQuota(provider: string, config: OcxProviderConfig): Promise<ProviderQuotaProbeResult> {
  let token = "";
  try {
    token = resolveDevinToken(config);
  } catch {
    token = "";
  }
  const quota = token ? await fetchDevinQuotaLive(token) : null;
  return quota ? report(provider, "devin-http:account", quota) : null;
}
...
export function keyQuotaReaderForProvider(name: string, provider: OcxProviderConfig): KeyQuotaReader | null {
  ...
  if ((name === "electronhub" || name === "electronhub-coding-plan")
    && isCanonicalElectronHubBaseUrl(provider.baseUrl)) return fetchElectronHubQuota;
  // devin-http holds its own session token for the chat path, so the seat-management RPC is
  // queried with that same credential instead of a separate account probe.
  if (provider.adapter === "devin-http") return fetchDevinHttpQuota;
  return null;
}
