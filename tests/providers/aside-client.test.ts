...
describe("Aside client config", () => {
  ...
  test("matches the provider shape observed in a real Aside catalog", () => {
    ...
    // `compat` rides along because Aside reuses the pi document verbatim: the same
    // developer-role pin that keeps routed models on `system` for pi applies here.
    expect(new Set(Object.keys(provider))).toEqual(new Set(["baseUrl", "apiKey", "api", "compat", "models"]));
