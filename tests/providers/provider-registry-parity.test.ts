...
const EXPECTED_KEY_PROVIDER_IDS = [
  "devin-http", "anthropic-apikey", "openai-apikey", "meta-model", "umans", "opencode-go", "neuralwatt", "openrouter",
  "cline-pass", "cline", "orcarouter", "packycode", "bizrouter", "groq", "google", "google-vertex", "azure-openai",
  "deepseek", "cerebras", "chutes", "deepinfra", "hyperbolic", "nscale", "vultr", "baseten", "commandcode",
  "sambanova", "nebius", "crusoe", "digitalocean", "scaleway", "featherless", "novita", "together", "fireworks",
  "firepass", "moonshot", "huggingface", "nvidia", "venice", "zai", "zhipu-bigmodel", "zhipu-bigmodel-coding",
  "zhipu-bigmodel-responses", "nanogpt", "synthetic", "siliconflow", "qwen-cloud", "tencent-coding-plan", "volcengine",
  "volcengine-coding-plan", "volcengine-agent-plan", "qianfan", "alibaba", "alibaba-token-plan",
...
describe("provider registry parity", () => {
  ...
  test("LiteLLM, opencode-free, mimo-free, and devin-http are the registry seeds with optional key authentication", () => {
    ...
    // devin-http is optional on purpose: the credential can live in the CLI's own login
    // cache (devin auth login, Free-plan eligible), which devin-http reads from credentials.toml,
    // so a pasted key must not be required.
    expect(optionalKeyProviders).toEqual(["devin-http", "litellm", "opencode-free", "mimo-free"]);
  });
