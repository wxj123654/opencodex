...
describe("Devin provider split: CLI bridge vs per-token API", () => {
  test("the devin entry bridges the ACP CLI, not an HTTP endpoint", () => {
    ...
    // Base ids verified LIVE against devin 3000.10.21 (2026-09-11); reasoning variants are
    // effort suffixes on the wire, folded into ladders (swe-2 + medium -> swe-2-medium).
    expect(entry.models).toEqual(["swe-2", "swe-1-7", "swe-1-7-lightning", "swe-1-6"]);
    expect(entry.modelReasoningEfforts).toEqual({
      "swe-2": ["medium", "high", "max"],
      "swe-1-7": ["medium"],
      "swe-1-7-lightning": ["medium"],
    });
    expect(entry.modelDefaultReasoningEfforts).toEqual({ "swe-2": "medium" });
    expect(entry.modelContextWindows?.["swe-2"]).toBe(262_000);
  ...
  test("both entries derive key-login, init, and dashboard presets", () => {
    ...
    expect(providerConfigSeed(registryEntry("devin")).models).toEqual(["swe-2", "swe-1-7", "swe-1-7-lightning", "swe-1-6"]);
  ...
  test("routes devin/<model> to the ACP adapter without an apiKey requirement", () => {
    const route = routeModel(devinConfig(), "devin/swe-2");
    expect(route.modelId).toBe("swe-2");
