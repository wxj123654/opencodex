...
## Или позвольте opencodex управлять блоком

Ручное слияние — не единственный путь. opencodex может стать владельцем блока
`providers.opencodex` в этом файле: он записывает блок за вас — включая уровни рассуждений
в виде `reasoning: true` и `thinkingLevelMap`, ограничивающего выбор уровней Pi реальной
лестницей каждой модели — и не трогает остальных провайдеров файла.

```bash
ocx integration client enable --client pi                          # принять и записать блок
ocx integration client enable --client pi --overwrite-conflict     # принудительно заменить расходившийся блок
ocx integration client status --client pi                          # current / stale / not installed
ocx integration client history --client pi                         # все записи, с op id
ocx integration client restore --op <opId>                         # откатить запись
ocx integration client disable --client pi                         # снять владение (блок остаётся)
```

Если существующий блок редактировался вручную и больше не совпадает с тем, что написал бы
opencodex, `enable` отказывает. `--overwrite-conflict` — аварийный люк, заменяющий блок
содержимым текущего каталога. Учтите: управляемый блок Pi НЕ обновляется автоматически
при `ocx sync` (сегодня это умеет только MiniMax Code). После изменения моделей, лестниц
или видимости заново выполните `enable --overwrite-conflict` — или действие Refresh /
Replace на странице Integrations панели управления, — чтобы привести блок в актуальное
состояние. `status` со значением `stale` и есть сигнал к действию. Полная семантика,
снимки и правила отката — в [руководстве по интеграциям](/guides/integrations/).
...
[Удалённый доступ](/ru/reference/configuration/server/#удалённый-доступ).
...
Поле `cost` записывается явными нулями в каждой строке. Причины две, по одной на каждую сторону: загрузчик models.json в pi не применяет документированное нулевое значение по умолчанию, а его калькулятор использования читает `model.cost.tiers` для каждого сообщения ассистента — строка без `cost` рушит сессию с ошибкой `Cannot read properties of undefined (reading 'tiers')`. А поскольку у opencodex нет
данных о ценах для маршрутизируемых моделей, нули к тому же честны: оценка на стороне клиента остаётся $0, а реальный учёт ведёт прокси. Если вы сливаете экспортированные блоки вручную, сохраняйте поле `cost` — не удаляйте его. `reasoning` в Pi — булево поле, тогда как каталог несёт целую лестницу effort, и
