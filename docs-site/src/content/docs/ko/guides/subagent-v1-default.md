...
4. **`agentTaskRecovery`를 켭니다.** 실험적 기능으로 기본값은 꺼져 있습니다. ChatGPT 백엔드를 거쳐 새로운 스폰 대부분을 복구하지만 할당량과 지연 시간이 늘고 문서화되지 않은 동작에 의존합니다. 메시지 유형의 후속 작업과 여러 부분으로 된 엔벌로프는 여전히 잃습니다.
